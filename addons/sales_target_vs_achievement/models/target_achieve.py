...
class TargetAchieve(models.Model):
    ...
    #added by me to create a field to show percentage
...
# added by me to compute the percentage from the total
    @api.depends('sale_user_id', 'time_span', 'team_id', 'user_target', 'person_achieved_amt')
    def _compute_percentage(self):
        for rec in self:
            ...
            for order in orders:
                if order.user_id.id == rec.sale_user_id.user_id.id:
                    rec.percentage += (order.amount_total / rec.user_target)*100
