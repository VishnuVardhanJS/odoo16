				...
				if line.is_downpayment == False and line.display_type == 'product':
					...
					for exception in exception_ids:
						...
						if exception.based_on == 'Products':
							...
							name = 'Commission Exception'  +' '+'with'+ ' "' + tools.ustr(exception.based_on_2) + ' "' +' '+'for'+' '+tools.ustr(exception.based_on) + ' "' + tools.ustr(exception.product_id.name) + '" @' + tools.ustr(commission_precentage) + '%'
						elif exception.based_on == 'Product Categories':
							categ_id = exception.categ_id.id
							name = 'Commission Exception'  +' '+'with'+ ' "' + tools.ustr(exception.based_on_2) + ' "' +' '+'for'+' '+tools.ustr(exception.based_on) + ' "'  + tools.ustr(exception.categ_id.name) + '" @' + tools.ustr(commission_precentage) + '%'
						elif exception.based_on == 'Product Sub-Categories':
							sub_categ_id = exception.sub_categ_id.id
							name = 'Commission Exception'  +' '+'with'+ ' "' + tools.ustr(exception.based_on_2) + ' "' +' '+'for'+' '+tools.ustr(exception.based_on) + ' "' + tools.ustr(exception.sub_categ_id.name) + '" @' + tools.ustr(commission_precentage) + '%'
						invoice_commission_data = {'name': name,
